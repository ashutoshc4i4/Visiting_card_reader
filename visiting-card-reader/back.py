...
GOOGLE_SHEET_ID = '1YDNbsHhi5mWQF-QS5RmkmsgHpg0jqgCqmBUiUQFVleA'
GOOGLE_CREDENTIALS_FILE = 'C:/Users/it/Downloads/visiting-card-reader-465216-aeb7ead2d371.json'
...
def upload_file():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            # Duplicate check (by email and phone) only for current user's collection
            email = extracted_data.get('email')
            phone = extracted_data.get('phone')
            if email or phone:
                duplicate_query = {
                    'scanned_by': session['user']
                }
                if email:
                ...
                if duplicate:
                    os.remove(filepath)
                    return jsonify({'success': False, 'error': 'Duplicate card detected. This card already exists in your collection.'}), 409
            ...
            try:
                append_card_to_google_sheet(extracted_data)
            except Exception as e:
...
def signup():
    if request.method == 'POST':
        ...
        if not re.fullmatch(r'\d{4}', employee_id):
            flash('Employee ID must be exactly 4 digits.', 'danger')
            return render_template('signup.html', name=name, employee_id=employee_id, email=email)
...
def append_card_to_google_sheet(card_data):
    scope = ['https://www.googleapis.com/auth/spreadsheets']
    creds = Credentials.from_service_account_file(GOOGLE_CREDENTIALS_FILE, scopes=scope)
    client = gspread.authorize(creds)
    sheet = client.open_by_key(GOOGLE_SHEET_ID).sheet1  # Use .worksheet('SheetName') if not the first sheet
    row = [
        card_data.get('name', ''),
